...
public abstract class BaseIntegrationPage extends BaseWebPage {
	...
	protected String getBasePath(PageParameters parameters) {
		String basePath = ISVApplication.get().getServerConfiguration().getAppDirectBaseUrl();
	...
	protected EventInfo readEvent(PageParameters parameters) {
		...
		AppDirectIntegrationAPI service = JAXRSClientFactory.create(basePath, AppDirectIntegrationAPI.class);
		EventInfo eventInfo = service.readEvent(token);
