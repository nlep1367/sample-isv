package com.appdirect.isv.wicket.pages.appdirect.billing;
...
import com.appdirect.isv.backend.integration.remote.vo.AccountInfo;
import com.appdirect.isv.backend.integration.remote.vo.AddonInstanceInfo;
...
import com.appdirect.isv.backend.integration.remote.vo.UsageItemBean;
...
public class UsagePage extends BaseWebPage {
	...
	public UsagePage(PageParameters parameters) {
		...
		final AccountBean accountBean = ISVApplication.get().getIsvService().readAccountByUserID(ISVSession.get().getCurrentUser().getId());
		...
		usageForm.add(new AjaxButton("report") {
			...
			protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
				AppDirectIntegrationAPI api = JAXRSClientFactory.create(accountBean.getAppDirectBaseUrl(), AppDirectIntegrationAPI.class);
